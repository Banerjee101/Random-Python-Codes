...
def l_curve(ifile,oidn,l, e_b, START, STOP): 
    ...
    lc_file = """batbinevt detmask= """+outdir+"""sw"""+oidn+"""master_qmap.mask infile= """+outdir+ifile+""" outfile="""+outdir+lc+""" \
              outtype=LC timedel=0 timebinalg=snr snrthresh=30.0 energybins="""+e_b+""" tstart= """+str(START)+""" tstop= """+str(STOP)+""" \
              weighted=YES clobber=YES """  
    print lc_file
    os.system(lc_file)
...
#~ TSTART = BAT_evt[0].header['TRIGTIME']
...
oidn = observations[0]
LC_file = fits.open(oidn+"/bat/event_2/BAT_1_15-150_v1.lc",'readonly')
time_r = LC_file[1].header['TIMEPIXR']

times_lc  =np.array( LC_file[1].data['TIME'])

time_del =np.array( LC_file[1].data['TIMEDEL'])

rate_bins = np.array( LC_file[1].data['RATE'])

rate_errors = np.array( LC_file[1].data['ERROR'])

start_times = times_lc - time_r*time_del
stop_times  = times_lc + (1-time_r)*time_del
time_cen  = times_lc + (0.5-time_r)*time_del
...
mv_results = "mv "+observations[0]+"/bat/event_2 "+observations[0]+"-resultsLC_snr"
mv_lc = "mv BAT_LC.qdp "+observations[0]+"-resultsLC_snr"
mv_pco = "mv BAT_LC.pco "+observations[0]+"-resultsLC_snr"
os.system(mv_results)
os.system(mv_lc)
os.system(mv_pco)

ascii.write([time_cen, start_times, stop_times,rate_bins,rate_errors], 'BAT_LC_times_snr30.qdp', names=['bin_center','start_times', 'stop_times', 'rate', 'error'])
mv_times = "mv BAT_LC_times_snr30.qdp "+observations[0]+"-resultsLC_snr"
os.system(mv_times)
